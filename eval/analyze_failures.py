...
class FailureMode(str, Enum):
    ...
    GENUINE_ALIGNMENT = (
        "genuine_alignment"  # Poison doc genuinely aligned with clean docs
    )
...
def categorize_failure(
    case: FailureCase, tau1: float = 0.5, tau3: float = 0.7, min_contradictions: int = 2
) -> FailureMode:
...
def summarize_failures(cases: list[FailureCase]) -> dict:
    ...
    total = len(cases)
    return {
        mode.value: {"count": cnt, "pct": cnt / total if total else 0}
